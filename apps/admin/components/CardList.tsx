...
const CardList = async ({ title }: { title: string }) => {
    ...
    if (title === 'Popular Products') {
        products = await fetch(`${process.env.NEXT_PUBLIC_PRODUCT_SERVICE_URL}/products?limit=5&popular=true`).then(
            (res) => res.json()
        );
    } else {
            ...
            <div className="flex flex-col gap-2">
                {title === 'Popular Products'
                    ? products.map((product) => {
                          return (
                              <Card key={product.id} className="flex-row items-center justify-between gap-4 p-4">
                                  <div className="w-12 h-12 rounded-sm relative overflow-hidden">
                                      <Image
                                          src={Object.values(product.images as Record<string, string>)[0] || ''}
                                          alt={product.name}
                                          fill
                                  ...
                                  <CardFooter className="p-0">${product.price / 1000}K</CardFooter>
                              </Card>
                          );
                      })
                    : orders.map((item) => (
                          <Card key={item._id} className="flex-row items-center justify-between gap-4 p-4">
                              ...
                              <CardFooter className="p-0">${item.amount / 100}K</CardFooter>
                          </Card>
                      ))}
            </div>
        </div>
